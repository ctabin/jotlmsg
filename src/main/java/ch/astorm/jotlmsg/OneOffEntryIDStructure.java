...
public class OneOffEntryIDStructure extends FlatEntryStructure {
	...
	private static final byte[] UUID={(byte) 0x81, (byte) 0x2B, (byte) 0x1F, (byte) 0xA4,
	...
	public OneOffEntryIDStructure(ByteBuffer bf) {
		...
		bf2.position(OneOffEntryIDStructure.FLAGS.length + OneOffEntryIDStructure.UUID.length + OneOffEntryIDStructure.VERSION.length + OneOffEntryIDStructure.PAD_MAE_FORMAT_M_U_R_L_PAD.length);
	...
	public OneOffEntryIDStructure(String displayName, String emailAddress) {
		...
		long totalSize = OneOffEntryIDStructure.FLAGS.length + OneOffEntryIDStructure.UUID.length + OneOffEntryIDStructure.VERSION.length + OneOffEntryIDStructure.PAD_MAE_FORMAT_M_U_R_L_PAD.length;
		...
		// Fill ByteBuffer.
        bf.put(OneOffEntryIDStructure.FLAGS).put(OneOffEntryIDStructure.UUID).put(OneOffEntryIDStructure.VERSION).put(OneOffEntryIDStructure.PAD_MAE_FORMAT_M_U_R_L_PAD);
